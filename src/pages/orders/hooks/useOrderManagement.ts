...
export function useOrderManagement(userRole: string) {
  ...
  // Track first render to prevent double loading
  const isFirstRender = useRef(true);

  // Load metrics wrapper - memoized to prevent memory leaks
  ...
  useEffect(() => {
    // Skip on first render to prevent double loading
  ...
  }, [loadMetricsWrapper]); // ✅ Now includes loadMetricsWrapper
