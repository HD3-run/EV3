...
import { createUser, findUserByEmail, findUserByEmailOrPhone, findUserById, verifyPassword } from './user.model';
import { logger } from './utils/logger';
import { pool } from './db';
import { createPhantomTokenPair } from './utils/jwt';
import ordersRouter from './orders';
import inventoryRouter from './inventory';
import reportsRouter from './reports';
import invoicesRouter from './invoices';
import billingDetailsRouter from './billing-details';
...
import { getPoolStatus } from './middleware/pool-protection';
...
// Register route
router.post('/register', validatePassword, async (req: Request, res: Response) => {
...
router.post('/login', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!isPasswordValid) {
      logger.warn('Failed login attempt - invalid password', { emailOrPhone: emailOrPhone.substring(0, 3) + '***' });
      return res.status(401).json({ message: 'Invalid credentials' });
    }
...
router.get('/validate-session', async (req: Request, res: Response) => {
  try {
    ...
    const user = await findUserById(req.session.userId.toString());
    if (!user) {
      // User not found, clear session
      req.session.destroy((err) => {
        if (err) logger.error('Session destruction error during validation', err);
      });
...
export function registerRoutes(app: Router) {
  ...
  app.get('/api/merchant-info', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const merchantResult = await client.query(
        'SELECT merchant_id, merchant_name FROM oms.merchants WHERE merchant_id = $1',
        [merchantId]
      );
      ...
      logger.error('Error fetching merchant info', error instanceof Error ? error.message : String(error));
  ...
  app.use('/api/billing-details', authenticateUser, billingDetailsRouter);
  app.use('/api/returns', authenticateUser, returnsRouter);
  app.use('/api/catalog', authenticateUser, productCatalogRouter); // Product catalog routes - requires authentication
  app.use('/api/public/catalog', publicCatalogRouter); // Public catalog routes - no authentication required
  app.use('/api/public/orders', publicOrdersRouter); // Public order creation - no authentication required
  app.use('/api/employee', authenticateUser, employeeRouter); // Employee routes - requires authentication
  
  // Add debug endpoints for development
  if (process.env.NODE_ENV === 'development') {
    // Pool status monitoring endpoint
    app.get('/api/debug/pool-status', async (req: Request, res: Response) => {
      try {
        const status = getPoolStatus();
        res.json(status);
      } catch (error) {
        logger.error('Error fetching pool status', error instanceof Error ? error.message : String(error));
  ...
  app.put('/api/profile/password', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!/[A-Z]/.test(newPassword)) errors.push('Password must contain uppercase letter');
      if (!/[a-z]/.test(newPassword)) errors.push('Password must contain lowercase letter');
      if (!/\d/.test(newPassword)) errors.push('Password must contain number');
      if (!/[!@#$%^&*]/.test(newPassword)) errors.push('Password must contain special character');
      ...
      const { hashPassword } = await import('./user.model');
  ...
  app.post('/api/users', async (req: Request, res: Response) => {
    ...
    let merchantId: string | number | null = null;
    let username: string = '';
    let email: string = '';
    let phone: string = '';
    let role: string = '';
    ...
    try {
      ...
      if (existingUser.rows.length > 0) {
        logger.warn('User creation failed: email already exists', { email: email.substring(0, 3) + '***' });
      ...
      const existingPhone = await client.query(
        'SELECT user_id FROM oms.users WHERE phone_number = $1',
        [phone]
      );
      
      if (existingPhone.rows.length > 0) {
        logger.warn('User creation failed: phone number already exists', { phone: phone.substring(0, 3) + '***' });
        return res.status(409).json({ message: 'User with this phone number already exists' });
      }
      
      // Check if username already exists for this merchant
      const existingUsername = await client.query(
        'SELECT user_id FROM oms.users WHERE merchant_id = $1 AND username = $2',
        [merchantId, username]
      );
      
      if (existingUsername.rows.length > 0) {
        logger.warn('User creation failed: username already exists for merchant', { username, merchantId });
      ...
      if (errorMessage.includes('duplicate key value violates unique constraint')) {
        if (errorMessage.includes('users_pkey')) {
          ...
          if (!merchantId || !username || !email || !phone || !role || !passwordHash) {
            logger.error('Missing required variables for sequence fix retry', {
          ...
          logger.warn('User ID sequence out of sync - attempting to fix automatically');
          try {
            ...
            logger.info(`Current max user_id in database: ${maxId} (type: ${typeof maxId})`);
            ...
            logger.info(`Starting search for available ID from ${nextAvailableId}`);
            ...
            const sampleCheck = await client.query(`
              ...
              LIMIT 10
            `, [maxId - 10]);
            logger.info(`Sample IDs around max:`, sampleCheck.rows.map((r: any) => `${r.user_id} (num: ${r.user_id_num})`));
            ...
            while (checkedIds < maxChecks) {
              ...
              if (idCheck.rows.length === 0) {
                ...
                if (count === 0) {
                  ...
                  logger.info(`Found available ID: ${nextAvailableId} after checking ${checkedIds} IDs (double-checked)`);
                  break;
                } else {
                  logger.warn(`ID ${nextAvailableId} exists (found by double-check, count=${count}), continuing search...`);
                }
              } else {
                logger.info(`ID ${nextAvailableId} exists: ${JSON.stringify(idCheck.rows[0])}`);
              }
              ...
              if (checkedIds % 100 === 0) {
                logger.info(`Checked ${checkedIds} IDs, current candidate: ${nextAvailableId}`);
              }
            }
            
            if (checkedIds >= maxChecks) {
              logger.error(`Could not find available ID after checking ${maxChecks} IDs. Max ID: ${maxId}`);
              return res.status(500).json({ 
            ...
            logger.info(`Next available user_id: ${nextAvailableId} (checked ${checkedIds} IDs)`);
            ...
            if (!sequenceName) {
              logger.warn('Sequence not found, attempting to create one');
              try {
                ...
                logger.info(`user_id default value: ${defaultValue}`);
                ...
                logger.info(`Created sequence ${sequenceName} starting at ${startValue}`);
              } catch (createError) {
                logger.error('Failed to create sequence', createError instanceof Error ? createError.message : String(createError));
                return res.status(500).json({ 
            ...
            try {
              ...
              logger.info(`Sequence ${sequenceName} current state:`, currentSeqValue.rows[0]);
              ...
              logger.info(`Sequence ${sequenceName} reset to ${nextAvailableId - 1} with is_called=true (next value will be ${nextAvailableId})`);
              ...
              logger.info(`Sequence ${sequenceName} after reset: last_value=${lastValue} (type: ${typeof lastValue}), is_called=${isCalled}, expected next=${expectedNext}`);
              
              if (expectedNext !== nextAvailableId) {
                logger.error(`Sequence will generate ${expectedNext} but we need ${nextAvailableId}. This is a critical error.`);
                throw new Error(`Sequence configuration failed: expected ${nextAvailableId} but sequence will generate ${expectedNext}`);
              }
              
              logger.info(`✓ Sequence correctly configured to generate ${nextAvailableId} on next call`);
            } catch (setvalError) {
              logger.error('Failed to set sequence value', setvalError instanceof Error ? setvalError.message : String(setvalError));
              ...
              try {
                ...
                logger.info(`Sequence reset using alternative method to ${nextAvailableId - 1}`);
              } catch (altError) {
                logger.error('Alternative setval also failed', altError instanceof Error ? altError.message : String(altError));
                throw setvalError; // Re-throw original error
              }
            }
            
            // Retry the insert - let database auto-generate the ID
            logger.info('Retrying user creation with auto-generated ID');
            ...
            if (finalCount > 0) {
              logger.error(`ID ${nextAvailableId} exists in database (count=${finalCount}), but our checks missed it. Finding next available...`);
              ...
              if (gapQuery.rows.length > 0) {
                ...
                logger.info(`Found actual available ID using gap query: ${actualAvailableId}`);
                
                // Insert with explicit ID
                const retryResult = await client.query(
                  'INSERT INTO oms.users (user_id, merchant_id, username, email, phone_number, password_hash, role) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING user_id, username, email, phone_number, role',
                  [actualAvailableId, merchantId, username, email, phone, passwordHash, role]
                );
                
                // Update sequence to be ahead of this ID
                await client.query(`SELECT setval($1, $2, true)`, [sequenceName, actualAvailableId]);
                logger.info(`User created successfully with explicit ID ${actualAvailableId}, sequence updated`);
                
                logger.info('User created successfully after sequence reset', { userId: retryResult.rows[0].user_id, createdBy: req.session.userId });
                return res.status(201).json({ 
                  message: 'User created successfully with custom password',
            ...
            try {
              ...
              if (insertError?.message?.includes('duplicate key value violates unique constraint')) {
                logger.warn(`INSERT failed with duplicate key even after sequence reset. Using gap query to find available ID...`);
                ...
                let actualAvailableId: number | null = null;
                let attempts = 0;
                ...
                while (!actualAvailableId && attempts < maxGapAttempts) {
                  ...
                  if (gapQuery.rows.length > 0) {
                    ...
                    logger.info(`Found actual available ID using gap query: ${actualAvailableId} (attempt ${attempts + 1}, searched ${startId}-${endId})`);
                    break;
                  }
                  
                  attempts++;
                  logger.warn(`No available ID found in range ${startId}-${endId}, trying next range...`);
                }
                ...
                if (actualAvailableId) {
                  ...
                  if (verifyCount > 0) {
                    logger.error(`ID ${actualAvailableId} exists (count=${verifyCount}) even though gap query said it doesn't! Trying next ID...`);
                  ...
                  // Insert with explicit ID
                  const explicitInsertResult = await client.query(
                    'INSERT INTO oms.users (user_id, merchant_id, username, email, phone_number, password_hash, role) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING user_id, username, email, phone_number, role',
                    [actualAvailableId, merchantId, username, email, phone, passwordHash, role]
                  );
            ...
            logger.error('Failed to fix sequence and retry', retryErrorMsg);
  ...
  app.get('/api/customers/:customerId', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const result = await client.query(
        'SELECT name, phone, email, address_line1, address_line2, landmark, city, state, pincode, delivery_note, state_code, gst_number FROM oms.customers WHERE customer_id = $1 AND merchant_id = $2',
      ...
      logger.error('Error fetching customer details', error instanceof Error ? error.message : String(error));
  ...
  app.post('/api/orders/assign', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const orderCheck = await client.query(
        'SELECT order_id, status, user_id FROM oms.orders WHERE order_id = $1 AND merchant_id = $2',
        [orderId, merchantId]
      );
      ...
      logger.info('About to update order', { orderId, userId, oldStatus, previousUserId, merchantId });
      ...
      try {
        ...
        await client.query(
          ...
          [orderId, `assignment:${userId}`, oldStatus, req.session.userId]
        );
        
        logger.info('Status history logged successfully', { assignedUserId: userId, previousUserId });
      ...
      const { invalidateUserCache } = await import('./middleware/cache');
      ...
      await client.query('ROLLBACK');
          ...
          logger.warn('Rollback failed (may not be in transaction)', rollbackError);
        }
      } catch (connectionError) {
        logger.warn('Database connection is dead, skipping rollback', connectionError);
      }
  ...
  app.get('/api/orders/:orderId/assignment-history', async (req: Request, res: Response) => {
    const client = await pool.connect();
    try {
      ...
      const currentAssignmentResult = await client.query(
        ...
        WHERE o.order_id = $1 AND o.user_id IS NOT NULL`,
        [orderId]
      );
      ...
      for (const row of historyResult.rows) {
        ...
        if (assignedUserIdMatch) {
          ...
          const assignedUserResult = await client.query(
            'SELECT username, role FROM oms.users WHERE user_id = $1',
            [assignedUserId]
          );
      ...
      logger.error('Error fetching assignment history', error instanceof Error ? error.message : String(error));
