...
export class BatchProcessor {
  ...
  constructor(client: PoolClient, options: Partial<BatchProcessingOptions> = {}) {
    this.client = client;
    this.options = {
      batchSize: 500,
      maxRetries: 3,
   ...
   */
  async processBatches<T extends BatchItem, R = any>(
    items: T[],
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      while (retryCount <= this.options.maxRetries && !batchSuccess) {
        try {
          ...
          if (this.options.enableProgressTracking && (global as any).io) {
  ...
  async processInventoryBatch(
    ...
    return this.processBatches(
      ...
      async (batch) => {
        ...
        const inventoryValues = productResults.rows.map((_, index) => 
  ...
  async processOrdersBatch(
    ...
    return this.processBatches(
      ...
      async (batch) => {
        ...
        const orderValues = batch.map((_, index) => 
  ...
  async processInvoicesBatch(
    ...
    return this.processBatches(
      ...
      async (batch) => {
        ...
        const invoiceValues = validInvoices.map((_, index) => 
