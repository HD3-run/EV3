...
// Get order metrics (total revenue, pending orders, today's orders)
router.get('/metrics', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [revenueResult, pendingResult, todayResult] = await Promise.all([
      // Total revenue from paid orders
      client.query(
        'SELECT COALESCE(SUM(total_amount), 0) as total_revenue FROM oms.orders WHERE merchant_id = $1 AND payment_status = $2',
        [merchantId, 'paid']
      ),
      // Count of pending payment orders
      client.query(
        'SELECT COUNT(*) as pending_count FROM oms.orders WHERE merchant_id = $1 AND payment_status = $2',
        [merchantId, 'pending']
      ),
      // Count of today's orders
      client.query(
        'SELECT COUNT(*) as today_count FROM oms.orders WHERE merchant_id = $1 AND DATE(created_at) = CURRENT_DATE',
        [merchantId]
      )
    ]);
    ...
    const metrics = {
      ...
      pendingOrders: parseInt(pendingResult.rows[0].pending_count) || 0,
      todayOrders: parseInt(todayResult.rows[0].today_count) || 0
    };

    logger.info('Metrics calculated', {
      merchantId,
      metrics,
      calculationMethod: 'server-side SQL aggregation'
    });
