...
router.get('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await client.query(
      'SELECT * FROM oms.merchant_billing_details WHERE merchant_id = $1',
      [merchantId]
    );
    ...
    logger.error('Error fetching billing details', sanitizeForLog(error instanceof Error ? error.message : String(error)));
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const existingResult = await client.query(
      'SELECT billing_id FROM oms.merchant_billing_details WHERE merchant_id = $1',
      [merchantId]
    );
    ...
    if (existingResult.rows.length > 0) {
      ...
      const updateResult = await client.query(
        `UPDATE oms.merchant_billing_details SET
         gst_number = $1,
         pan_number = $2,
      ...
      logger.info('Billing details updated', { merchantId, billingId: billingDetails.billing_id });
    } else {
      console.log('📝 Creating new billing details...');
      // Create new billing details
      console.log('🔄 Executing INSERT query...');
      const insertResult = await client.query(
        `INSERT INTO oms.merchant_billing_details 
      ...
      logger.info('Billing details created', { merchantId, billingId: billingDetails.billing_id });
    }
    ...
    logger.error('Error saving billing details', error instanceof Error ? error.message : String(error));
