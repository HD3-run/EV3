...
// Import queries
import { getProductsWithFilters, getLowStockProducts, getUserMerchantId } from './inventory/queries/product-queries';

// Import services
import { createBasicProductService, updateProductService, addProductService } from './inventory/services/productService';
import { bulkUpdateInventoryService, updateStockService, updateProductAndInventoryService } from './inventory/services/inventoryService';
import { updatePriceService, updateSellingPriceService } from './inventory/services/priceService';
import { processStockUpdateCSV, processProductCSV } from './inventory/services/csvService';
...
router.get('/', validatePagination, cacheMiddleware(60), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const merchantId = await getUserMerchantId(client, (req as any).session.userId);
    if (!merchantId) {
    ...
    const result = await getProductsWithFilters(client, merchantId, {
      page: Number(page),
      limit: limitNum,
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const product = await createBasicProductService(client, (req as any).session.userId, {
      name,
      sku,
      description,
      category,
...
router.put('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const product = await updateProductService(client, (req as any).session.userId, parseInt(id), {
      name,
      sku,
      description,
      category,
...
router.get('/low-stock', cacheMiddleware(30), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await getLowStockProducts(client, merchantId);
...
router.post('/bulk-update', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const results = await bulkUpdateInventoryService(client, (req as any).session.userId, updates);
...
router.post('/update-stock-csv', upload.single('file'), async (req: Request, res: Response) => {
  logger.info('POST /api/inventory/update-stock-csv - Request received', {
  ...
  try {
    if (!req.file) {
      logger.error('No file uploaded for stock update');
      return res.status(400).json({ message: 'No file uploaded' });
    }

    const result = await processStockUpdateCSV(client, (req as any).session.userId, req.file.buffer, uploadId);
    ...
    logger.error('Error processing stock update CSV upload - DETAILED', error instanceof Error ? error.message : String(error));
...
router.post('/upload-csv', upload.single('file'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await processProductCSV(client, (req as any).session.userId, req.file.buffer, uploadId);
...
router.patch('/:id/price', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await updatePriceService(client, (req as any).session.userId, parseInt(id), unitPrice);
    ...
    try {
      ...
      if (io) {
        ...
        logger.info('WebSocket event emitted for inventory price update', { productId: id, unitPrice });
      }
    } catch (error) {
      logger.warn('Could not emit WebSocket event for inventory price update', { error: error instanceof Error ? error.message : String(error) });
    }
...
router.patch('/:id/selling-price', async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    const { id } = req.params;
    const { sellingPrice } = req.body;
    
    await updateSellingPriceService(client, (req as any).session.userId, parseInt(id), sellingPrice);
    ...
    try {
      ...
      if (io) {
        ...
        logger.info('WebSocket event emitted for inventory selling price update', { productId: id, sellingPrice });
      }
    } catch (error) {
      logger.warn('Could not emit WebSocket event for inventory selling price update', { error: error instanceof Error ? error.message : String(error) });
    }
    
    res.json({ message: 'Selling price updated successfully' });
  } catch (error) {
    logger.error('Error updating product selling price', error instanceof Error ? error.message : String(error));
    const statusCode = error instanceof Error && error.message.includes('not found') ? 404 : 
                      error instanceof Error && error.message.includes('Valid') ? 400 : 500;
...
router.post('/add-product', validateQuantity, async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.info('Extracted request data', { name, category, stock, reorderLevel, unitPrice, sellingPrice });
    
    const result = await addProductService(client, (req as any).session.userId, {
      name,
      category,
      brand,
      description,
    ...
    try {
      ...
      if (io) {
        ...
        logger.info('WebSocket event emitted for new product', { productId: result.productId, productName: name });
      }
    } catch (error) {
      logger.warn('Could not emit WebSocket event for new product', { error: error instanceof Error ? error.message : String(error) });
    }
...
router.patch('/:id/stock', validateQuantity, async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    const { id } = req.params;
    const { quantity } = req.body;

    const result = await updateStockService(client, (req as any).session.userId, parseInt(id), quantity);
    ...
    try {
      ...
      if (io) {
        ...
        logger.info('WebSocket event emitted for stock update', { productId: id, quantity });
      }
    } catch (error) {
      logger.warn('Could not emit WebSocket event for stock update', { error: error instanceof Error ? error.message : String(error) });
    }

    res.json({ message: 'Product stock updated successfully :', product: result });
  } catch (error) {
    logger.error('Error updating product stock manually', error instanceof Error ? error.message : String(error));
    const statusCode = error instanceof Error && error.message.includes('not found') ? 404 :
                      error instanceof Error && error.message.includes('Valid') ? 400 : 500;
...
router.patch('/:id/update', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await updateProductAndInventoryService(client, (req as any).session.userId, parseInt(id), {
      productName,
      brand,
      description,
      quantity,
    ...
    try {
      ...
      if (io) {
        ...
        logger.info('WebSocket event emitted for inventory update', { productId: id });
      }
    } catch (error) {
      logger.warn('Could not emit WebSocket event for inventory update', { error: error instanceof Error ? error.message : String(error) });
    }
